...
func (h *SteamApiHandler) GetOwnedGames(ctx *gin.Context, options structs.BannerOptions) func() {
	return func() {
		filter := func(gameId int) string {
			if options.BoxArt == true {
...
func (h *SteamApiHandler) GetGameHeaders(ctx *gin.Context) {
	h.GetOwnedGames(ctx, structs.BannerOptions{BoxArt: false})()
}

func (h *SteamApiHandler) GetGameBoxart(ctx *gin.Context) {
	h.GetOwnedGames(ctx, structs.BannerOptions{BoxArt: true})()
}
